...
class RRT:
    ...
    def obstacle_check(self, point1, point2, obsList, buffer=0):
        goodPoint = True
        for obs in obsList:
            try:
            ...
            if (distInt <= obs[2]+buffer) and (max(abs(point1[0]), abs(point2[0])) > abs(intersect[0]) > min(abs(point1[0]), abs(point2[0]))):
                goodPoint = False
                break
            elif (distPoint1 <= obs[2]+buffer) or (distPoint2 <= obs[2]+buffer) :
                goodPoint = False
                break
        return goodPoint
    ...
    #         else:
    #             goodPoint = True
    #     slopeRad = math.atan2((-closestPoint[1]+randomPoint[1]), (-closestPoint[0]+randomPoint[0]))
    #     print(slopeRad)
